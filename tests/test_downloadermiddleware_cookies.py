...
class CookiesMiddlewareTest(TestCase):
    ...
    def test_dont_merge_cookies(self):
        ...
        req = Request('http://scrapytest.org/dontmerge',
                      meta={'dont_merge_cookies': 1})
        assert self.mw.process_request(req, self.spider) is None
        assert 'Cookie' not in req.headers
    ...
    def test_complex_cookies(self):
        ...
        assert req.headers.get('Cookie') in (
            b'C1=value1; C3=value3', b'C3=value3; C1=value1')
    ...
    def test_merge_request_cookies(self):
        ...
        self.assertCookieValEqual(req2.headers.get(
            'Cookie'), b"C1=value1; galleta=salada")
    ...
    def test_cookiejar_key(self):
        ...
        res2 = Response('http://scrapytest.org/',
                        headers=headers, request=req3)
        assert self.mw.process_response(req3, res2, self.spider) is res2

        req4 = Request('http://scrapytest.org/', meta=res2.meta)
        assert self.mw.process_request(req4, self.spider) is None
        self.assertCookieValEqual(req4.headers.get(
            'Cookie'), b'C2=value2; galleta=dulce')
        ...
        headers = {'Set-Cookie': 'C1=value1; path=/'}
        res5_1 = Response('http://scrapytest.org:1104/',
                          headers=headers, request=req5_1)
    ...
    def test_local_domain(self):
        request = Request("http://example-host/",
                          cookies={'currencyCookie': 'USD'})
    ...
    def test_keep_cookie_from_default_request_headers_middleware(self):
        ...
        mw_default_headers = DefaultHeadersMiddleware(
            DEFAULT_REQUEST_HEADERS.items())
    ...
    def test_keep_cookie_header(self):
        ...
        req2 = Request('http://scrapytest.org',
                       headers={'Cookie': 'a=b; c=d'}, cookies={'e': 'f'})
        assert self.mw.process_request(req2, self.spider) is None
    ...
    def test_request_cookies_encoding(self):
        ...
        req2 = Request('http://example.org',
                       cookies={'a': 'á'.encode('latin1')})
        assert self.mw.process_request(req2, self.spider) is None
    ...
    def test_request_headers_cookie_encoding(self):
        # 1) UTF8-encoded bytes
        req1 = Request('http://example.org',
                       headers={'Cookie': 'a=á'.encode('utf8')})
        assert self.mw.process_request(req1, self.spider) is None
        self.assertCookieValEqual(req1.headers['Cookie'], b'a=\xc3\xa1')

        # 2) Non UTF8-encoded bytes
        req2 = Request('http://example.org',
                       headers={'Cookie': 'a=á'.encode('latin1')})
        assert self.mw.process_request(req2, self.spider) is None
        self.assertCookieValEqual(req2.headers['Cookie'], b'a=\xc3\xa1')
